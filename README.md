...
**より積極的で賢い売買**を実現するため、以下の機能が実装されています：

1. **動的信頼度計算**
   - AI判断に技術指標（RSI、モメンタム、ボリューム）を組み合わせて信頼度を動的に調整
   - 信頼度が `min_confidence` 以下の場合は取引をスキップ

2. **技術指標による売買シグナル補強**
   - **RSI**: 30以下で買いシグナル、70以上で売りシグナル
   - **モメンタム**: SMA(5) - SMA(20) でトレンド方向を判定
   - **ボリューム**: 平均の1.5倍以上で確信度アップ
   - **ボラティリティ**: 高ボラ時は信頼度を下げてリスク管理

3. **拡張AI応答パース**
   - シンプルなテキスト形式（`BUY`, `SELL 0.8`）に加え、JSON形式もサポート
   - JSON例: `{"action": "BUY", "confidence": 0.8, "reasoning": "..."}`

4. **設定可能なパラメータ**
   ```yaml
   strategies:
     ai_hybrid:
       min_confidence: 0.6          # 最小信頼度閾値 (0.0-1.0)
       aggressive_mode: true         # true=積極的、false=保守的
       use_technical_signals: true   # 技術指標による検証を有効化
   ```

5. **トレード情報の充実**
   - 各トレード判断に「AI信頼度」「最終信頼度」「RSI」「トレンド」を表示
   - ログで詳細な判断根拠を確認可能
